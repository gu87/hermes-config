...
- `skills` — 子 Agent 可用的 skill 白名单。从 2026-05-25 起，所有 8 个 Agent 的 `subagent_profile.skills` 数组已全部填充。验证方法：读 agent-registry.json 中每个 Agent 的 subagent_profile.skills 数组 → 与 SKILL.md frontmatter 的 `agents:` 标签交叉校验。
...
| 失败降级 | 如果 CLI 工具连续失败，降级为 Agent TARS / Intelligence / browser 等当前备选 |
...
| OpenClaw | 旧版角色遗留在当前编制/路由中 | 移除当前路由，桌面能力迁到 `agent-tars`，调研能力迁到 `intelligence` |

### 10. 角色审计：type/capabilities 常见错误

| 错误模式 | 问题 | 修复 |
|---------|------|------|
| type 与手册角色不匹配 | 桌面/调研混在同一 Agent | 桌面给 `agent-tars`，调研给 `intelligence` |
...
### 18. 调研 Agent 工具集缺失导致调研失败

**问题**：派 Intelligence 去搜图/搜资料时，如果只能用 curl，容易被 CDN 拦截（403/404），没法浏览网页。

**根因**：`agent-registry.json` 中 intelligence 的 toolsets 没有 `web` 和 `browser`。
...
**验证方法**：搜图任务 `delegate_task(agent_id='intelligence', goal='搜真实照片')` 应该能用 web/browser 工具直接访问网页。
...
**真实案例（2026-05-21 MiniMax 删除）**：内斯塔尝试用 `patch` 删除 `.env` 中的 `MINIMAX_CN_API_KEY` 和 `MINIMAX_CN_BASE_URL` 两行，返回拒绝。父 Agent 必须用 `terminal` 工具手动清理：
```bash
sed -i '' '/^MINIMAX_CN/d' /Users/gu/.hermes/.env
```

**适用场景**：任何涉及 `.env` 文件修改的 delegation（API key 轮换、provider 添加/删除等）。

**建议做法**：
- 子 Agent 完成 config.yaml 和 auth.json 的修改后，在 context 里注明「.env 文件需要手动清理」
- 父 Agent 用 terminal 执行 `sed` 或手动编辑`.env`
- 验证：`grep -i <keyword> .env && echo "STILL HAS" || echo "CLEAN"`
...
### 21. Managed Agent 风险预检拒绝（risk R0）

**症状**：`delegate_task(agent_id='claude', goal='部署/编译任务')` 返回错误：
```
Managed agent preflight rejected delegation: Agent claude cannot handle risk R0
```

**真实案例（2026-05-23 CodexPlusPlus 部署）**：
- 用户要求用 cc（Claude Code）部署 Rust + Tauri 项目
- `delegate_task(agent_id='claude')` 被 risk 预检拒绝
- 切换到 `agent_id='deepseek-tui'` 成功派发
- 但 deepseek-tui 在 `cargo build --release`（全量 Rust 编译，~5-15 分钟）上被父 Agent dialog_timeout 截断

**根因**：系统对 agent-registry.json 中每个 Agent 有风险标签。Claude Agent（file_executor）被标记为低风险——只能处理单文件修改、简单脚本。涉及编译、部署、系统配置变更的任务归类为更高风险。风险预检是确定性的——同一 agent 重试必再次被拒。

**回退链路**：

```
被拒 agent_id='claude'
  → 换 agent_id='deepseek-tui'（编码 agent，terminal 权限更宽）
    → 如子 Agent 超时，主会话用 background terminal + watcher 接管
```

**规则**：
- 风险预检拒绝 ≠ 模型配置错（tokens > 0 可区分）
- 不要重试同一个被拒的 agent_id
...
**真实案例（2026-05-23 CodexPlusPlus 部署）**：
- deepseek-tui 子 Agent 成功执行：`npm install`（3s）、`npm run vite:build`（1s）
- `cargo build --release` 启动后 112s，父 Agent 超时截断子 Agent 会话
- 编译进程（PID 67116）继续在后台运行
- 父 Agent 通过 `ps aux | grep cargo` 确认进程存活
- 设置文件存在性 watcher 等待产物

**模式**：

```bash
# 子 Agent 中启动编译 → 子 Agent 超时 → 主会话接管监控
# 1. 确认进程还在
ps aux | grep "cargo build"
# 2. 设文件 watcher
while sleep 30; do
  [ -f target/release/codex-plus-plus-manager ] \
    && echo "=== BUILD COMPLETE ===" \
    && ls -lh target/release/codex-plus-plus* \
    && break
done
# 3. 验证产物可执行
...
**推荐做法**：
1. 子 Agent 中只启动编译命令，不等待完成，返回 PID
2. 父 Agent 用 `ps aux` 确认进程存活
3. 父 Agent 设后台 watcher 检测产物文件
4. 产物出现后执行验证

**不推荐**：
- 子 Agent 中 `process('wait', timeout=600)` 等编译——子 Agent 自己的 dialog_timeout 先到
- 重试同一个子 Agent——每个新子会话看不到前一个的背景进程，子 Agent 只能重新编译
- 子 Agent 启动编译后让它 `notify_on_complete=True`——子 Agent 退出时编译不一定完成
...
- 从 2026-05-25 起，所有 8 个 Agent 的 `subagent_profile.skills` 和 SKILL.md `agents:` 标签已双向对齐
- 新增/修改 skill 时，必须同步更新 skill frontmatter 和 agent-registry.json 的 skills 数组
...
- `references/multi-agent-snapshot-diagnostic.md` — 多 Agent 系统快照诊断工作流：7 步排查网关、进程、端口、日志、cron、DNS、资源，附带输出模板和 pitfalls
...
- `references/long-running-build-subagent.md` — 子 Agent 启动长编译/部署任务后被超时截断的处理模式：进程存活确认、文件存在性 watcher、产物验证。对应 Pitfall 21-22
