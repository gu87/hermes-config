...
| **自己动手做技术调研/代码研究** | **当任务涉及技术调研（读源码、定位文件、理解架构）时，先派给内斯塔预处理，不要自己动手读代码。内斯塔擅长快速扫描项目结构、定位相关文件、打包精确上下文给执行 Agent。你（马蒂尼）的职责是协调和验收，不是一线做技术探索。Gu 的明确要求：「你的角色不是自己干，你把这些交给内斯塔」** |
