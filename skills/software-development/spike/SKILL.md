...
### Reverse-engineering undocumented APIs from JS bundles

When automating a web app that has no public API documentation, the JS bundle is the source of truth. This technique applies whenever you need to call purchase/order/subscription APIs that aren't documented externally.

**Standard approach:**

1. **Find the JS bundle URL** — most SPA apps serve a single `app.[hash].js` from the root. The `Content-Security-Policy` or nginx config in the HTML source tells you the CDN domain.

2. **Fetch the bundle** — `curl -s "https://domain.cn/js/app.[hash].js"`. Save it; you need to search it extensively.

3. **Find API paths** — search for URL strings containing the API prefix:
   ```python
   import re
   paths = re.findall(r'url:["\'](/[a-zA-Z0-9/_-]{5,})["\']', js)
   unique = sorted(set(paths))
   for p in unique:
       if any(k in p for k in ['subscribe', 'purchase', 'order', 'pay', 'product']):
           print(p)
   ```

4. **Find function call signatures** — search for the function name (e.g., `createBankOrder`) and read the 200-500 chars before/after to see what parameters are passed:
   ```python
   for m in re.finditer(r'createBankOrder', js):
       snippet = js[max(0, m.start()-200):m.start()+300]
       print(snippet)
   ```

5. **Find component API calls** — look for the route name (e.g., `SubscribePay`) to find the chunk file it loads, then fetch that chunk:
   ```python
   # From the route definition:
   # n.e("SubscribePay") loads chunk "c749"
   curl -s "https://domain.cn/js/c749.js"
   ```

6. **Map response paths** — when an API returns 404, look at the `path` field in the error JSON. The path shown is after the gateway prefix, so strip the first prefix (e.g., `/biz/tokenAccounts/purchase` means the actual backend path is `/tokenAccounts/purchase`).

7. **Common pitfalls:**
   - Chrome cookies can't be used directly as Bearer tokens — cookie auth and token auth are separate systems. Use the login API to get a token.
   - JS bundles are heavily minified — use Python regex on the raw text, not a JS parser.
   - Bundle hash changes on every deploy — if a URL stops working, re-fetch the JS bundle and re-analyze.
   - `/api/` prefix doesn't always mean the same as `baseURL` in the code — always verify with actual API calls.

**Token storage convention:** when getting tokens via login API, store at `/tmp/{provider}_token.txt` for quick retrieval in subsequent script runs.

**References:**
- `references/zhipuai-api-2026-04.md` — reverse-engineering Zhipu GLM Coding Plan purchase API (login flow, working/broken endpoints, JS bundle analysis, 2026-04 session)
