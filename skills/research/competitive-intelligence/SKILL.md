...
### Primary Tool: AnySearch MCP (2026-06-08+)
...
3. **Harvest OpenChronicle event logs for real-time internal intelligence** — When OpenChronicle is running (screen-capture plugin), the daily event log (`event-YYYY-MM-DD.md`) under `~/Library/Mobile Documents/iCloud~md~obsidian/Documents/个人知识库/3-知识/wiki/OpenChronicle记忆/` contains raw Feishu chat captures, MCN platform activity, and internal brand discussions. This is a **primary offline intelligence source** that web search cannot match:
   ...
   a. **Read today's event log first** — use `read_file` to scan for brand keywords in recorded Feishu messages (耐克、蒙牛、伊利、安慕希、百威、小米、比亚迪、剑南春、世界杯、赞助). The event log captures visible_text from Feishu group chats even when you lack web/browser tools.
      ...
      - **Fallback when `mcp_openchronicle_search_captures` is unavailable** (verified 2026-06-05): If the OpenChronicle MCP tool is not in your toolset, read today's `event-YYYY-MM-DD.md` directly with `read_file`, then run `search_files(target='content', pattern='<brand keywords>')` across the OpenChronicle memory directory. This produced a complete 14-item execution briefing in this session without any MCP search calls.
...
**⚠️ Bing browser-search quirk — 国内版拆词 & 国际版弱搜索** (verified 2026-06-03): When `browser_navigate` lands on cn.bing.com (国内版), English queries are aggressively split into individual words — "World Cup 2026" becomes "word" + "cup" + "2026", returning dictionary definitions rather than news. Switching to 国际版 doesn't fully fix this; even 国际版 may return mostly dictionary/definition results for multi-word English queries.

**Fix:** Do NOT use Bing browser search (`browser_navigate` + `browser_type` + `browser_press`) as a primary entry point for English-language queries. If you must use Bing via browser, (a) use quoted phrases like `"World Cup"`, (b) prefer `browser_navigate` directly to the Bing News URL with `qft` parameters rather than typing into the search box, or (c) use Chinese keywords only.
...
- **MiniMax web_search API rate limit — DISABLED 2026-06-08**: The `mcp_minimax_web_search` MCP has been disabled (mcp_servers.minimax.enabled = false). Do not attempt to use it. Use AnySearch MCP (`mcp_anysearch_search`) instead.
...
| 36氪 (36kr.com) | `36kr.com/search/articles/<encoded-query>` | High | **React SPA — curl 不可用，必须用 browser_navigate + browser_snapshot。** 搜索页直接返回带摘要和标题的搜索结果列表，点击文章可获取全文（含发布时间、作者、正文）。**当 Baidu News 触发 CAPTCHA、Google 被阻、MiniMax API 超额时，36kr 是中文营销情报最可靠的回退入口。** 搜索技巧：使用品牌精确关键词（如「蒙牛 世界杯」）而非宽泛词（如「世界杯 营销」），宽泛词按时间排序会混入大量不相关文章。已验证 2026-06-02：蒙牛、小红书版权、车企、义乌等世界杯相关文章均可成功提取。注意：直接 curl 只返回空 HTML 壳，不能用 terminal 抓取 |
...
| Sportcal | `sportcal.com/news/...` | Medium (listing-only) | Sports business trade (GlobalData). **Listing pages only** — `/news/`, `/sector/soccer/` are accessible via Scrapling basic mode and show article headlines. Individual article pages require login (member-login redirect). Use listing page headlines + snippets for briefing content; do NOT attempt to deep-read individual Sportcal articles. Verified 2026-06-03. |
...
- **Intelligence-agent-first delegation for extreme network constraint** (verified 2026-06-03): When ALL main-agent external tools are dead (MiniMax rate-limited + Baidu CAPTCHA + Google/Bing timeout + AdAge/SportBusiness blocked), delegate to Intelligence agent IMMEDIATELY as the PRIMARY research track — do NOT try to fix external sources first. Intelligence's internal-source scan (OpenChronicle event logs + Obsidian vault + Feishu captures) can independently produce a complete 13+ item briefing. Run this in parallel with the main agent doing 36kr browser search (the only reliable external survivor). This dual-track approach produced a full briefing in today's session with zero working web_search/browser/scrapling calls on the main agent side. The Intelligence agent used only file tools (skill_view, read_file, search_files) — no web_search needed.

- **External source progressive degradation — minimum viable set** (verified 2026-06-03 evening): When the main agent's external tools degrade progressively (MiniMax rate-limited → Baidu CAPTCHA → Google timed out → Bing browser search dysfunctional), the surviving external sources narrow to exactly two: **SportsPro listings** (sportspro.com/news) and **Sportcal listings** (sportcal.com/news). SportsPro article pages remain fully readable; Sportcal is listing-only (login wall on articles). Together they provide 2-3 actionable stories for a daily briefing. This pattern was confirmed on 2026-06-03 evening — the briefing was built entirely from SportsPro articles (Zee deal + CL final piracy) + Sportcal headlines (Curry-Li-Ning), supplemented by Intelligence agent internal sources. Do NOT waste turns re-trying dead endpoints; pivot to SportsPro/Sportcal after the first full round of external failures.

- **Intelligence agent timeout when all external sources fail** (verified 2026-06-02): When web_search, browser_navigate, and scrapling_fetch ALL fail due to network restrictions (rate limits, CAPTCHAs, timeouts), an Intelligence subagent may burn its full 180s timeout re-trying dead endpoints (25+ API calls) and return nothing — OR (verified 2026-06-03) it may succeed brilliantly by switching to internal-source-only mode. The outcome depends on whether the agent recognizes the dead-end early. **Mitigation**: Include explicit "if web_search returns rate-limit error, skip all web tools and switch to internal sources (OpenChronicle + Obsidian)" in the goal instructions. Prefer the dual-track strategy (Intelligence for internal + main agent for 36kr) over single-channel delegation.
- **Scrapling stealth mode timeout on AdAge/The Drum/SportBusiness** (verified 2026-06-02, 2026-06-03): Scrapling `s_fetch_page` with `mode=stealth` consistently times out (30s+) on AdAge, The Drum, and SportBusiness category pages. These trade publications should be treated as snippet-only or accessed via `browser_navigate` directly. Do not waste Scrapling calls on these domains.
...
**Playwright MCP — DISABLED 2026-06-08**: `mcp_servers.playwright.enabled = false`. Do not use browser_navigate or Playwright. Use AnySearch MCP for search + extract. Scrapling stealth mode remains available as a fallback for specific trade publications (Marketing Brew, SportsPro).
...
- **Desktop directory `search_files` glob searches timeout — use content regex instead** (verified 2026-06-05): On macOS, `search_files(target='files', pattern='*世界杯*', path='/Users/gu/Desktop')` repeatedly timed out after 60s due to the large file count. Switching to `search_files(target='content', pattern='世界杯|百威|蒙牛|比亚迪|赞助')` returned instantly with actionable matches. **Rule of thumb**: for directories with many items (Desktop, Downloads, large project roots), prefer `target='content'` with a regex over `target='files'` with a glob.
...
- `references/world-cup-2026-06-02-evening-session.md` — 2026-06-02晚报session记录：极端网络限制下36kr browser search为唯一幸存数据源；小红书版权实锤、蒙牛暗战、车企阅兵等关键发现；macOS grep -P不兼容教训
- `references/world-cup-2026-06-03-evening-session.md` — 2026-06-03晚报session记录：极限外部源退化模式（仅剩SportsPro+Sportcal listing）、Bing浏览器搜索国内版拆词陷阱、Sportcal listing-only新源确认、Scrapling stealth SportBusiness超时
- `references/world-cup-2026-06-03-morning-session.md` — 2026-06-03早报：全工具链几乎全灭环境下的双轨策略（Intelligence agent内部源扫描 + 主Agent 36kr幸存者搜索），13条纯内部情报验证离线模式可独立产报
- `references/world-cup-2026-06-02-morning-session.md` — 2026-06-02早报session记录
- `references/world-cup-2026-06-01-evening-session.md` — 2026-06-01晚报session记录：Bing News+scrapling stealth = 最佳英文实时发现源（5h前Forbes文章）、中文Bing News系统性空结果、Intelligence agent委托陷阱、Google News RSS仅适用于档案研究
